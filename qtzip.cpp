...
public:
    ...
    QtZipReader::FileInfo fillFileInfo(int index) const;
...
QtZipReader::FileInfo QtZipPrivate::fillFileInfo(int index) const
{
    ...
    case HostVFAT:
        ...
        break;
    default:
        qWarning("QtZip: Zip entry format at %d is not supported.", index);
...
class QtZipReaderPrivate : public QtZipPrivate
{
public:
    QtZipReaderPrivate(QIODevice *device, bool ownDev)
        : QtZipPrivate(device, ownDev), status(QtZipReader::NoError)
    {
    }
...
void QtZipReaderPrivate::scanFiles()
{
    if (!dirtyFileTree)
        return;

    if (! (device->isOpen() || device->open(QIODevice::ReadOnly))) {
        status = QtZipReader::FileOpenError;
        return;
    }

    if ((device->openMode() & QIODevice::ReadOnly) == 0) { // only read the index from readable files.
        status = QtZipReader::FileReadError;
        return;
    }
    ...
    while (start_of_directory == -1) {
        ...
        if (pos < 0 || i > 65535) {
            qWarning("QtZip: EndOfDirectory not found");
            return;
        }
    ...
    if (comment_length != i)
        qWarning("QtZip: failed to parse zip file.");
    ...
    for (i = 0; i < num_dir_entries; ++i) {
        ...
        if (read < (int)sizeof(CentralFileHeader)) {
            qWarning("QtZip: Failed to read complete header, index may be incomplete");
            break;
        }
        if (readUInt(header.h.signature) != 0x02014b50) {
            qWarning("QtZip: invalid header signature, index may be incomplete");
            break;
        }
        ...
        if (header.file_name.length() != l) {
            qWarning("QtZip: Failed to read filename from zip index, index may be incomplete");
            break;
        }
        l = readUShort(header.h.extra_field_length);
        header.extra_field = device->read(l);
        if (header.extra_field.length() != l) {
            qWarning("QtZip: Failed to read extra field in zip file, skipping file, index may be incomplete");
            break;
        }
        l = readUShort(header.h.file_comment_length);
        header.file_comment = device->read(l);
        if (header.file_comment.length() != l) {
            qWarning("QtZip: Failed to read read file comment, index may be incomplete");
            break;
        }
...
void QtZipWriterPrivate::addEntry(EntryType type, const QString &fileName, const QByteArray &contents/*, QFile::Permissions permissions, QtZip::Method m*/)
{
#ifndef NDEBUG
       ...
       ulong len = contents.length();
        ...
        do {
            ...
            case Z_OK:
                ...
                break;
            case Z_MEM_ERROR:
                qWarning("QtZip: Z_MEM_ERROR: Not enough memory to compress file, skipping");
                data.resize(0);
                break;
            case Z_BUF_ERROR:
                len *= 2;
...
/*!
    ...
    \brief the QtZipReader class provides a way to inspect the contents of a zip
    archive and extract individual files from it.

    QtZipReader can be used to read a zip archive either from a file or from any
 ...
 */
QtZipReader::QtZipReader(QIODevice *device)
    : d(new QtZipReaderPrivate(device, /*ownDevice=*/false))
{
    Q_ASSERT(device);
}
...
*/
QIODevice* QtZipReader::device() const
{
    return d->device;
}
...
*/
bool QtZipReader::isReadable() const
{
...
/*!
    ...
    \sa fileInfoList()
*/
QtZipReader::FileInfo QtZipReader::entryInfoAt(int index) const
{
    d->scanFiles();
    if (index >= 0 && index < d->fileHeaders.count())
        return d->fillFileInfo(index);
    return QtZipReader::FileInfo();
}

/*!
    Fetch the file contents from the zip archive and return the uncompressed bytes.
*/
QByteArray QtZipReader::fileData(const QString &fileName) const
{
    d->scanFiles();
    int i;
    ...
    if ((general_purpose_bits & Encrypted) != 0) {
        qWarning("QtZip: Unsupported encryption method is needed to extract the data.");
        return QByteArray();
    }
    ...
    if (compression_method == CompressionMethodStored) {
        ...
        do {
            ...
            case Z_OK:
                ...
                break;
            case Z_MEM_ERROR:
                qWarning("QtZip: Z_MEM_ERROR: Not enough memory");
                break;
            case Z_BUF_ERROR:
                len *= 2;
                break;
            case Z_DATA_ERROR:
                qWarning("QtZip: Z_DATA_ERROR: Input data is corrupted");
                break;
            }
        } while (res == Z_BUF_ERROR);
        return baunzip;
    ...
    qWarning("QtZip: Unsupported compression method %d is needed to extract the data.", compression_method);
    return QByteArray();
}
...
*/
bool QtZipReader::extractAll(const QString &destinationDir) const
{
...
/*!
    Returns a status code indicating the first error that was met by QtZipReader,
    or QtZipReader::NoError if no error occurred.
*/
QtZipReader::Status QtZipReader::status() const
{
    return d->status;
}
...
/*!
    Returns a status code indicating the first error that was met by QtZipWriter,
    or QtZipWriter::NoError if no error occurred.
*/
...
{
    d->addEntry(QtZipWriterPrivate::File, QDir::fromNativeSeparators(fileName), data);
}
...
void QtZipWriter::addFile(const QString &fileName, QIODevice *device)
{
    ...
    d->addEntry(QtZipWriterPrivate::File, QDir::fromNativeSeparators(fileName), device->readAll());
    if (opened)
        device->close();
}
...
{
    d->addEntry(QtZipWriterPrivate::Symlink, QDir::fromNativeSeparators(fileName), QFile::encodeName(destination));
}
...
void QtZipWriter::close()
{
    ...
    //qDebug("QtZip::close writing directory, %d entries", d->fileHeaders.size());
